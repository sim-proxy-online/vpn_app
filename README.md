...
- **Подписки** — автообновление, поддержка HWID-locked, любых провайдеров подписок!
...
- **Bypass-пинг** — при выключенном VPN замеряет латентность через fragment/REALITY. Работает на DPI-сетях
- **Last-known кэш** — и на строгом глушении показывает реальный пинг bypass-серверов вместо прочерков
- **Не-bypass сервера** — корректно показывают прочерк, а не ложные данные
- **Fast-path** — при повторном открытии списка пинги появляются мгновенно
